...
function createHeatMap(data) {
  ...
  const cellHeight = (height - (hPadding * 2)) / 12;
  const cellWidth = (width - wPadding) / (maxYear - minYear);
  ...
  const yScale = d3.scaleBand()
                    .domain(d3.range(12))
                    .range([hPadding, height - hPadding])
  ...
  // const legend = tempColors.map((color, i) => {
  //   return { color: color, temp: ((i+1) * rangeLength).toFixed(1)}
  // })
  ...
  legend.attr("transform",`translate(${wPadding}, 0)`)
}
